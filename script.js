...
function updateCourseStates() {
    ...
    document.querySelectorAll('.curso').forEach(curso => {
        ...
        const data = allCourses[code];
        const prereqs = data.prerreqs;

        if (completed.has(code)) {
            curso.className = "curso completado";
            completados += data.creditos;
    ...
    const porcentaje = ((completados / totalCreditos) * 100).toFixed(1);
    document.getElementById("progreso").textContent =
        '✔️ Has completado ' + completados + ' créditos de ' + totalCreditos +
        ' totales (' + porcentaje + '%)';
